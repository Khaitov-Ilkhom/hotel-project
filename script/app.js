...
const generateId = () => {
    return String(Math.floor(Math.random() * 1000)).padStart(5, "0")
}

function Room(roomNumber, roomType, roomFloor, roomCap, roomPrice){
    this.id = generateId()
...
const createNewForm = (e) => {
    ...
    let isRoomExist = allRooms.find(room => room.roomNumber === Number(roomNumberInput.value));
...
const addEventToElement = (e) => {
    if(e.target.classList.contains("delete-room-btn")){
        ...
        if(id){
            addRoomForm.classList.add("show");
            roomNumberInput.value = editedRoom.roomNumber;
